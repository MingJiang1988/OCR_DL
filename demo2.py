...
def mnist_tensor(image_path):
    str = digits_model.getDigit(image_path)
    return str  # first value in list
...
if __name__ == '__main__':
    ...
    sess_alpha = tf.Session(graph=model_alpha, config=config)
    sess_digit = tf.Session(graph=model_digit)

    net = get_network("VGGnet_test")

    with model_alpha.as_default():
        model_saver = tf.train.Saver()
    with tf.Session(graph=model_alpha) as sess:
        model_saver.restore(sess)
        ckpt = tf.train.get_checkpoint_state(cfg.TEST.checkpoints_path)
        print('Restoring from {}...'.format(ckpt.model_checkpoint_path), end=' ')
        tf.global_variables_initializer().run()
        model_saver = tf.train.Saver(tf.global_variables())
        model_saver.restore(sess_alpha, ckpt.model_checkpoint_path)
        im = 128 * np.ones((300, 300, 3), dtype=np.uint8)
        for i in range(2):
            _, _ = test_ctpn(sess_alpha, net, im)
